...
class ApprovePeminjamanPengembalianController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $peminjaman = Sirkulasi::latest()->with('Buku', 'User')
                // ->whereBetween('status', [Sirkulasi::STATUS_MENUNGGU_PERSETUJUAN , Sirkulasi::STATUS_APPROVAL_ADMIN, Sirkulasi::STATUS_PENGEMBALIAN_APPROVAL_ADMIN])
                ->whereIn('status', [
                    Sirkulasi::STATUS_MENUNGGU_PERSETUJUAN,
                    Sirkulasi::STATUS_APPROVAL_ADMIN,
                    Sirkulasi::STATUS_PENGEMBALIAN_APPROVAL_ADMIN,
            ...
            return DataTables::of($peminjaman)
                ...
                ->addColumn('status', function ($row) {
                    ...
                    } elseif ($row->status == Sirkulasi::STATUS_APPROVAL_ADMIN) {
                        return '<div class"row"><div class="col-md-12 d-flex justify-content-center"><span class="badge badge-success">Approve</span></div></div';
                    } elseif ($row->status == Sirkulasi::STATUS_PENGEMBALIAN_APPROVAL_ADMIN) {
                        return '<div class"row"><div class="col-md-12 d-flex justify-content-center"><span class="badge badge-primary">Telah dikembalikan</span></div></div';
                ...
                ->addColumn('aksi', function ($row) {
                    if ($row->status == Sirkulasi::STATUS_MENUNGGU_PERSETUJUAN) {
                        ...
                        $button = '<div class="row">
                                        <div class="col-12 d-flex justify-content-center">
                                            <button type="button" title="Approve Peminjaman" data-id="' . base64_encode($row->id) . '" class="btn btn-success btn-sm mt-1 mb-1 ml-1 mr-1 btn-open-modal-approve-pengembalian"><i class="fas fa-thumbs-up"></i></button>
                                        </div>
                                    </div>';
                        return $button;
                    } else {
    ...
    {
        $response = Sirkulasi::AdminApprovePengembalian($request);
        return $response->getData();
    }
